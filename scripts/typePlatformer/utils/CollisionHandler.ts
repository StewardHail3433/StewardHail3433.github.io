...
export class CollisionHandler {
    ...
    public update(entities: Entity[], chunks: Map<string, WorldTile[][]>, dt: number) {
        for(let i = 0; i < entities.length; i++) {
            ...
            if(entities[i] instanceof Player) this.handleAxisCollision("x", entities[i], chunks);
            this.handleAxisEntityMovement("y", entities[i], dt)
            if(entities[i] instanceof Player) this.handleAxisCollision("y", entities[i], chunks);
        }
    }
